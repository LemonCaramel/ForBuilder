...
import moe.caramel.forbuilder.Main;
...
public record EventListener(@NotNull Plugin plugin) implements Listener {
    ...
    void onBlockBreak(BlockBreakEvent event) {
        ...
        if (event.getBlock().getWorld().getKey().equals(OVER_WORLD)) event.setCancelled(true);
    }
    ...
    void onPlayerTakeLecternBook(PlayerTakeLecternBookEvent event) {
        if (!event.getPlayer().isSneaking()) event.setCancelled(true); // Toggle Sneaking
    }
}
